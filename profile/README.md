# QA 校友项目

欢迎访问 QA 校友项目！

## 我们正在维护

### QA瓜田

QA瓜田是我们维护的面向青岛中学和金家岭学校学生的校园论坛，拥有全面的功能和优秀的隐私保护，目前版本为V4（已开源）

你可以访问 https://qaguatian.com/ 使用本项目。

V1 - V3 版本现已无法访问，但你可以在[此链接](https://v3.qaguatian.com/)中查看历史数据。

### 技术文档

一个帮助大家快速掌握基本编程技能的文档：https://docs.yxzl.dev/

## 我们

- [异想之旅のBlog](https://blog.yxzl.dev/)
- [个人主页 | Honahec](https://honahec.cc/)
- [Modenc](https://modenc.top/)
- [Iron_Grey_](https://irongrey.top/)

## 联系

你可以：

- [发布一个 Issue](https://github.com/QingdaoAcademy/.github/issues/new)
- 邮件：hcc@qdzx.icu（也可以联系在上面“我们”中列出的任意成员的邮箱）
